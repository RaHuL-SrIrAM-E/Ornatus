...
Exercises the full loop —
    user -> agent -> tools (context/weather/wardrobe/preferences) ->
    reasoning -> outfit recommendation -> decision log -> feedback ->
    preference signal -> next recommendation
...
def test_outfit_recommendation_end_to_end(
    ...
    # Cool Friday evening + business-casual dinner, no preferences learned
    # yet -> the blazer gets picked.
    ...
    assert recommendation.excluded_item_ids == []
...
def test_full_learning_loop_avoids_previously_rejected_item(
    ...
    This is the Phase 1 learning-loop milestone: the second recommendation
    for the identical request must not repeat the item the user rejected,
    and must still be a complete, valid outfit (not just "the blazer
    ...
    # The preference was actually learned, not just the feedback recorded.
    preferences = preference_service.get_preferences(DEMO_USER_ID)
    assert any(
        p.type == PreferenceType.ITEM_DISLIKE and p.value == "item-blazer-navy" for p in preferences
    )
    ...
    assert second.decision.decision_type == DecisionType.OUTFIT_RECOMMENDATION
    assert "get_user_preferences" in second.decision.tools_used
    # The blazer is excluded — not recommended, and recorded as such.
    assert "item-blazer-navy" not in second.decision.selected_item_ids
    assert "item-blazer-navy" in second.decision.excluded_item_ids
    # Still a real, complete recommendation with other real items.
    assert second.decision.selected_item_ids
    for item_id in second.decision.selected_item_ids:
        assert wardrobe_service.get_item(item_id) is not None

    second_recommendation = outfit_service.get_latest_for_user(DEMO_USER_ID)
    assert second_recommendation.excluded_item_ids == ["item-blazer-navy"]
    assert second_recommendation.preferences_considered

    # The reply explains the choice in plain language, not as a database lookup.
